...
class Node:
    def __init__(self, node_name, node_ping, node_load):
        self.name = node_name
...
print "Modes:"
print "[0] Analyze nodes"
print "[1] Generate node list"
...
if mode == 0:
    ...
    for node in node_nodes:
        ...
        if connection is not None:
            ...
            nodes.append(Node(hostname, ping, load))
        else:
    ...
    with open(output, 'w') as log:
        for node in nodes:
            log.write(node.name + ":" + str(node.ping) + ":" + str(node.load) + '\n')
